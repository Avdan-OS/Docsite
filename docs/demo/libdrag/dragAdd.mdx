...
```js
dragAdd = (e, currentTarget, target, currentTarget_in_f, target_in_f, currentTarget_f, target_f, currentTarget_out_f, target_out_f, drop_f, modify_X = true, modify_Y = true, extraX, extraY)
```

dragAdd builds and `info` Object and pushes it in `dragList`. This `info` Object contains all information to manipulate the drag process.
...
currentTarget.addEventListener("mousedown", e => {
    dragAdd(e, `#${currentTarget.id}`, `#${target.id}`);
});
```
...
```js
currentTarget.addEventListener("mousedown", e => {
    dragAdd(e, `#${currentTarget.id}`, `#${target.id}`, undefined, dropTransition, undefined, undefined, undefined, returnExample);
});
```
