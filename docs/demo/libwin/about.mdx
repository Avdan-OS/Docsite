...
Dock underlines are connected with windows via `win_id` id, that is used while generation in `Appbar` and extra functions via `Window.inc()`. 

Every window generation means incrementing this variable (`win_did`).
